...
- Delegation defaults to read-only, Consult-managed confinement. Built-in
  `codex` and `claude` Profiles are confined on native Linux and Apple Silicon
  macOS after an
...
- Custom and `opencode` Profiles currently require explicit inheritance.
  Confined nested delegation, native Windows, and Intel macOS (including
  inheritance) are unsupported.
