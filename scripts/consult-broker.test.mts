...
const fakeAgentPath = fileURLToPath(
  new URL("./lib/__fixtures__/fake-acp-agent.mts", import.meta.url),
);
...
test("consult/ping returns broker health before the ACP agent is connected", async (t) => {
  ...
  try {
    const response = (await client.request("consult/ping", {})) as Record<string, unknown>;
...
test("consult/run fails a Copilot turn from a binary older than the supported floor", async (t) => {
  const harness = await startBroker(t, {
    profile: "copilot",
    agentArgs: ["sessions", "prompt-copilot-old-version"],
  });
  const client = await connectBroker(harness.endpoint);
  const finalizedPromise = nextNotification(client, "consult/finalized");
  ...
  try {
    ...
    const finalized = await finalizedPromise;
    assert.equal(finalized.stopReason, "failed");
    assert.match(finalized.errorMessage, /COPILOT_VERSION_UNSUPPORTED/u);
    assert.match(finalized.errorMessage, /1\.0\.50/u);
  } finally {
    await client.close();
  }
});
...
test("foreground delegate persists a complete finalized job record", async (t) => {
  ...
  let releaseBrokerReplace!: () => void;
  let brokerReplaceFinished!: () => void;
  let brokerReplaceDelayed = false;
  const brokerReplaceDelay = new Promise<void>((resolve) => {
    releaseBrokerReplace = resolve;
  });
  const brokerReplaceComplete = new Promise<void>((resolve) => {
    brokerReplaceFinished = resolve;
  });

  t.mock.method(fsp, "rename", async (fromPath: fs.PathLike, toPath: fs.PathLike) => {
...
test("consult/ping bypasses BROKER_BUSY while a prompt turn is in flight", async (t) => {
  ...
  try {
    ...
    const response = (await pingClient.request("consult/ping", {}, { timeoutMs: 100 })) as Record<string, unknown>;
    assert.equal(response.ok, true);
    assert.equal(response.profile, "codex");
    assert.equal((await finalizedPromise).jobId, "job-1");
  } finally {
...
test("malformed socket messages return a parse error without crashing the broker", async (t) => {
  ...
  try {
    ...
    const client = await connectBroker(harness.endpoint);
    try {
      assert.equal(((await client.request("consult/ping", {})) as Record<string, unknown>).ok, true);
    } finally {
      await client.close();
    }
  } finally {
...
test("originator disconnect mid-prompt taints the broker when the agent does not acknowledge cancel", async (t) => {
  ...
  try {
    ...
    const ping = (await pingClient.request("consult/ping", {}, { timeoutMs: 100 })) as Record<string, unknown>;
...
interface StartBrokerOptions {
  ...
  captureCancels?: boolean;
  captureClientCalls?: boolean;
  captureMethods?: boolean;
  fakeTargetPath?: string;
  fakeTargetRelative?: string;
...
async function readClientObservations(clientLog: string): Promise<any[]> {
  const content = await fsp.readFile(clientLog, "utf8");
  return content.trim().split("\n").filter(Boolean).map((line) => JSON.parse(line));
}

async function readMethodLog(methodLog: string): Promise<any[]> {
  const content = await fsp.readFile(methodLog, "utf8");
  return content.trim().split("\n").filter(Boolean).map((line) => JSON.parse(line));
}

function nextNotification(client: TestBrokerClient, method: string): Promise<any> {
...
function collectNotifications(client: TestBrokerClient, method: string): any[] {
  const notifications: any[] = [];
  client.on(method, (params) => notifications.push(params));
  return notifications;
}

async function promptCount(promptLog: string): Promise<number> {
  const content = await fsp.readFile(promptLog, "utf8");
  return content.trim().split("\n").filter(Boolean).length;
}

async function cancelCount(cancelLog: string): Promise<number> {
  let content: string;
  try {
...
async function readJobRecord(harness: BrokerHarness, jobId: string): Promise<Record<string, any> | null> {
  try {
...
async function assertNotClosedWithin(promise: Promise<unknown>, timeoutMs: number): Promise<void> {
  let timeout: NodeJS.Timeout | undefined;
  try {
