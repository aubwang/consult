...
test("request rejects with BROKER_TIMEOUT when the server never replies", async (t) => {
  ...
  try {
    await assert.rejects(client.request("hang", {}, { timeoutMs: 100 }), (error) => {
...
test(
  ...
  async (t) => {
    ...
    await assert.rejects(client.request("drop", {}), (error) => {
...
test("connectBroker rejects with BROKER_UNREACHABLE when the socket path does not exist", async () => {
  ...
  await assert.rejects(connectBroker(socketPath, { connectTimeoutMs: 100 }), (error) => {
...
test("request rejects with BROKER_CLOSED after close", { timeout: 500 }, async (t) => {
  ...
  await assert.rejects(client.request("ping", {}), (error) => {
    assert.equal(error.code, "BROKER_CLOSED");
    return true;
  });
});
