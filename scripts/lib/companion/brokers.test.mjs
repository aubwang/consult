...
import { brokerFilePath, brokersDir } from "../broker-endpoint.mjs";
import { runBrokers } from "./brokers.mjs";
...
test("brokers lists running, stale, and malformed broker state", async (t) => {
  ...
  assert.deepEqual(
    JSON.parse(result.stdout).map(({ jobId, status }) => ({ jobId, status })),
    [
...
async function writeBrokerState(workspaceRoot, state) {
  const filePath = brokerFilePath({ workspaceRoot, ...state });
  await fs.mkdir(path.dirname(filePath), { recursive: true });
  await fs.writeFile(filePath, `${JSON.stringify(state)}\n`, "utf8");
  return filePath;
}

async function writeRawBroker(workspaceRoot, jobId, content) {
