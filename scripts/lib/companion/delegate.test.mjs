...
test("delegate background writes a queued record and spawns the task worker", async (t) => {
  ...
  const record = JSON.parse(await fs.readFile(path.join(jobsDir(workspaceRoot), "job-bg.json")));
  assert.equal(result.exitCode, 0);
...
test("delegate defaults direct CLI use to terminal/default host identity", async (t) => {
  ...
  const resultPromise = runDelegate({
    ...
    deps: quietDeps({
      ...
      loadOverride: async () => null,
      loadProfiles: async () => profilesFixture(),
      ensureBrokerSession: async (args) => {
        ensureArgs = args;
        return { client };
      },
...
async function writeParentJob(workspaceRoot, fields) {
  await fs.mkdir(jobsDir(workspaceRoot), { recursive: true });
  await fs.writeFile(
...
function agentTextUpdate(text, jobId = "job-happy") {
  return {
    jobId,
    sessionId: "session-1",
    update: {
