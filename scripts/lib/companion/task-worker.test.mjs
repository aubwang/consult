...
import { runTaskWorker } from "./task-worker.mjs";
...
async function readJob(workspaceRoot, jobId) {
  return JSON.parse(await fs.readFile(path.join(jobsDir(workspaceRoot), `${jobId}.json`), "utf8"));
}
