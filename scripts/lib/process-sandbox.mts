...
export type AgentSandboxMode = "off" | "bwrap";

type BindAccess = "ro" | "rw";
...
export function normalizeAgentSandbox(value: unknown): AgentSandboxMode {
...
export function buildAgentLaunch({
  ...
  const binds = new Map<string, BindEntry>();
...
function executableRoots(binary: string): string[] {
  const roots = [];
...
function parentDirectories(targets: string[]): Set<string> {
