...
import { brokerFilePath, brokersDir } from "../../../scripts/lib/broker-endpoint.mts";
import { handleSessionLifecycleHook } from "./session-lifecycle-hook.mts";

const hookScriptPath = fileURLToPath(new URL("./session-lifecycle-hook.mts", import.meta.url));
    ...
    await handleSessionLifecycleHook("SessionEnd", {
      ...
      deps: {
        ...
        teardownBrokerSession: async (input) => {
          tornDown.push(input as typeof tornDown[number]);
          return { teardown: "noop" as const, brokerFile: "" };
        },
      },
    });
  ...
  try {
    ...
    await handleSessionLifecycleHook("SessionEnd", {
      ...
      deps: {
        resolveWorkspaceRoot: async () => workspaceRoot,
        teardownBrokerSession: async (input) => {
          tornDown.push((input as { profile: string }).profile);
          return { teardown: "noop" as const, brokerFile: "" };
        },
      },
    });
...
async function writeBrokerState(
  workspaceRoot: string,
  state: { jobId: string; host: string; hostSessionId: string; profile: string },
): Promise<void> {
  await fsp.writeFile(
