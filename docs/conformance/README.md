...
| [copilot](copilot.md) | PASS | AUTH-DEFERRED | AUTH-DEFERRED | AUTH-DEFERRED | AUTH-DEFERRED | AUTH-DEFERRED | — | rejected by policy | 2026-08-17 handshake-level pass at `@github/copilot` 1.0.80 (preview support until the matrix completes); inherit-only with `--deny-tool` authority pins and a 1.0.60 version floor, unauthenticated preflight and model-error turns fail closed, confined preflight fails closed. |
